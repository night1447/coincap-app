...
const CoinInterface: FC<CoinProps> = ({id}) => {
        ...
        return (
            <Section>
                <Button type={'button'} variant={'error'} onClick={goBackHandler} className={styles.back}>Назад</Button>
                <div className={styles.head}>
                    <div className={styles.inner}>
                        ...
                        <div>
                            <Typography type={'h1'} className={styles.name}>{coin.name} ({coin.symbol})</Typography>
                            <Typography type={'h2'} className={styles.price}>
                                {getRoundingNumber(+coin.priceUsd)}{getCurrency()}
                                <span
                                    className={`${styles.price_value} ${coin.changePercent24Hr[0] === '-' ? styles.negative : styles.positive}`}>
                    ...
                    <ul className={styles.info}>
                        <li className={styles.item}>
                            ...
                            <Typography type={'h3'}
                                        className={styles.value}>{getRoundingNumber(+coin.marketCapUsd)}{getCurrency()}</Typography>
                        </li>
                        <li className={styles.item}>
                            <Typography type={'p'}>Volume (24Hr)</Typography>
                            <Typography type={'h3'}
                                        className={styles.value}>{getRoundingNumber(+coin.volumeUsd24Hr)}{getCurrency()}</Typography>
                        </li>
                        <li className={styles.item}>
                            <Typography type={'p'}>Supply</Typography>
                            <Typography type={'h3'}
                                        className={styles.value}>{getRoundingNumber(+coin.supply)}{getCurrency()}</Typography>
                        </li>
                    </ul>
                </div>
                <Graphic id={id}/>
