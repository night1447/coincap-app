...
const BuyingInterface: FC<BuyingInterfaceProps> = ({coin}) => {
    ...
    const [messageSettings, setMessageSettings] = useState<IMessageSettings>(initialState);
    ...
    return (
        <form onSubmit={submitFormHandler} className={styles.form}>
            <Typography type={'h2'} className={styles.title}>Добавление в кошелек</Typography>
            ...
            <div className={styles.wrapper}>
                <Button variant={'accent'} type={'button'} isCircle={true} onClick={decreaseValueHandler}>-</Button>
                ...
                <Button variant={'accent'} type={'button'} isCircle={true} onClick={increaseValueHandler}>+</Button>
            </div>
            <div className={styles.total}>
                <Typography type={'h3'}>Общая сумма</Typography>
                <Typography type={'p'}
                            className={styles.total_price}>{`${getRoundingNumber(value * +coin.priceUsd)}`}$</Typography>
            </div>
            {messageSettings?.showMessage ?
                <Message type={messageSettings.type}
                         onClose={closeMessageHandler}>{messageSettings.message}</Message> : <></>}
