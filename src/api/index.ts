...
type IInterval = 'm1' | 'm5' | "m15" | "m30" | "h1" | "h2" | "h6" | "h12" | "d1";
...
const $api = {
    ...
    async getHistory(id: string, interval?: IInterval): Promise<IHistory[]> {
        try {
            const response = await axios.get(`${this.baseURL}/${id}/history?interval=${interval || 'h12'}`);
        ...
        try {
            const response = await axios.get(`${this.baseURL}${createSettings(settings)}`);
            return response.data.data;
        } catch (e) {
            throw new Error('Error in request');
        }
